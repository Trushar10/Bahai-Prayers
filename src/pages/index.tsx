...
export default function Home() {
	...
	const [selectedPrayer, setSelectedPrayer] = useState<PrayerEntry | null>(
		null
	);
	...
	const fetchPrayerContent = async (
		slug: string
	): Promise<PrayerEntry | null> => {
		try {
	...
	const handleClick = async (slug: string) => {
		...
		if (prayer) {
			...
			setCurrentView('prayer');
			const originalSlug =
				typeof prayer.fields.slug === 'string'
					? prayer.fields.slug
					: String(prayer.fields.slug);
	...
	const obligatory = Object.keys(groupedPrayers).find((name) =>
		name.toLowerCase().includes('obligatory')
	);
	const general = Object.keys(groupedPrayers).find((name) =>
	...
	const orderedSections = [obligatory, general, ...rest, other].filter(
				...
				>
					...
					<div className="page-view">
							...
							{orderedSections.map((sectionName) => (
								...
								>
									...
									<div className="post-list">
										{groupedPrayers[sectionName].map(
											(p: PrayerEntry) => (
												<div
													key={p.sys.id}
													className="post-item"
						...
						<footer className="footer">
							<p>
								&copy; {new Date().getFullYear()} Prayer App.
								All rights reserved.
							</p>
						</footer>
					</div>
					...
					<div className="page-view">
						{selectedPrayer && (
							<>
								...
								<footer className="footer">
									<p>
										&copy; {new Date().getFullYear()} Prayer
